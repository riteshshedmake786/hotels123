    ...
    <div class="d-flex align-items-center flex-wrap mr-1">
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex align-items-center font-weight-bold my-2">
          ...
          <!--end::Item-->
          <!--begin::Item--><span class="label label-dot label-sm bg-dark opacity-75 mx-3"></span> <a href="#" class="-dark text-hover-dark opacity-75 hover-opacity-100">
          Hotel Details                           </a>
          <!--end::Item-->
...
<div class=" container ">
          ...
          <div class="card-body">
            ...
            <div class="example">
              <ul class="nav nav-tabs" id="myTab" role="tablist">
               <li class="nav-item">
                <a class="nav-link" id="Capacity"  href="{{url('admin/hotel/get_single_hotels_includes/'.$hotels_sub_entity_id)}}" aria-controls="profile"> <span class="nav-icon"><i class="flaticon2-layers-1"></i></span> <span class="nav-text">Capacity</span> </a>
              </li>
              <li class="nav-item">
                <a class="nav-link active" id="Menu"  href="{{url('admin/hotel/menu/'.$hotels_sub_entity_id)}}" aria-controls="profile"> <span class="nav-icon"><i class="flaticon2-layers-1"></i></span> <span class="nav-text">Menu</span> </a>
              </li>
            </ul>
      ...
      <div class="modal fade" id="addMenuViewModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
        <div class="modal-dialog  modal-lg modal-dialog-scrollable" role="document">
          <div class="modal-content">
           ...
           <div class="modal-body">
             <form action="{{url('admin/hotel/submitIncludeMenu')}}" method="post" enctype="multipart/form-data" id="submitIncludeMenuForm">
              {{ csrf_field() }}
            <input type="hidden" name="hotels_sub_entity_id" value="{{$hotels_sub_entity_id}}">
              <div class="row">
                <div class="form-group validated col-md-3">
                  ...
                  <input type="text" placeholder="Enter title" class="form-control" id="menu-title" name="title[]" >
                </div>
                <div class="form-group col-md-4" id="website_input">
                  <label for="exampleTextarea">Image</label>
                  <input type="file" class="form-control" id="menu-image" name="image[]" accept=".jpg,.jpeg,.png">
                </div>
                <div class="form-group col-md-4" id="website_input">
                  <label for="exampleTextarea">File</label>
                  <input type="file" class="form-control" id="menu-pdf" name="doc_file[]" accept=".pdf">
                </div>
  ...
  <div class="modal fade" id="menuEditModal" tabindex="-1" role="dialog" data-backdrop="static"
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Menu Form</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                   <i aria-hidden="true" class="ki ki-close"></i>
                 </button>
            </div>
   ...
   <script type="text/javascript">
    $(document).ready(function() {
      ...
      $('#submitIncludeMenuForm').submit(function(){
        console.log('hi');
            valid = true;

            if($("#menu-title").val() == ''){
              swal.fire('Please enter the title', 'Required error', 'error');
               valid =  false;
               return valid;
            }

            if($("#menu-image").val() == ''){
              swal.fire('Please select the .png, .jpg, .jpeg file', 'Required error', 'error');
               valid =  false;
               return valid;
            }

            if($("#menu-title").val() == '' || $("#menu-image").val() == '' || $("#menu-pdf").val() == ''){
              swal.fire('Please select the .pdf file', 'Required error', 'error');
               valid =  false;
               return valid;
            }
      ...
      $('#menu-image').change(function() {
         var names = $('#menu-image')[0].files[0].name;

          if(!/(\.jpeg|\.png|\.jpg)$/i.test(names)) {
             $('#menu-image').val('');
             swal.fire('Please select the .png, .jpg, .jpeg file only', 'Required error', 'error');
             return false;
          }
       });

      $('#menu-pdf').change(function() {
         var names = $('#menu-pdf')[0].files[0].name;

          if(!/(\.pdf)$/i.test(names)) {
             $('#menu-pdf').val('');
             swal.fire('Please select the .pdf file only', 'Required error', 'error');
             return false;
          }
       });
    });
...
if(counter >= 0){
  ...
  function dltTrTrabgle(v){
    ...
    var id = $id;
    console.log(id);
    $.post("editFormHotelsIncludeMenu", {id: id , _token: '{!! csrf_token() !!}'})
    .done(function( data ) {
        if(data.html != '') {
...
$(document).on('click', '#saveEdit', function(){
  ...
  if (title == "") {
    ...
    swal.fire('Please select the image', 'Required error', 'error');
  }
  else if (doc_file == ""){
    swal.fire('Please select the PDF file', 'Required error', 'error');
  } else {
        $.post( "submitFormHotelsIncludeMenu", { id: id, title:title, image: image,doc_file:doc_file, _token: '{!! csrf_token() !!}' })
        .done(function( data ) {
...
function deleteMenu($id){
    var id = $id;
    console.log(id);
    $.post("deleteAdminHotelsIncludeMemu", {id: id , _token: '{!! csrf_token() !!}'})
    .done(function( data ) {
      location.reload();
    })
    .fail(function() {
