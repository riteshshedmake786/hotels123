            ...
            <div class="card card-custom gutter-b example example-compact">
               <div class="card-header">
                  <h3 id="form-heading" class="card-title">
                     Add Hotel Key Attribute
                  </h3>
               </div>
               <!--begin::Form-->
               <form class="form" method="post" action="{{ url('admin/key_attributes/add') }}" id="form1" enctype="multipart/form-data">
                  {{ csrf_field() }}
                  ...
                  <div class="card-body">
                     ...
                     <div class="form-group">
                        ...
                        <div class="custom-file">
                            ...
                            <label class="custom-file-label" for="image"  value="{{ old('image') }}">Choose file</label>
            ...
            <div class="card card-custom gutter-b" >
               <div class="card-header flex-wrap py-3">
                  <div class="card-title">
                     <h3 class="card-label">Key Attributes List</h3>
                  </div>
               </div>
               <div class="card-body">
                  <!--begin: Datatable-->
                  <table class="table table-bordered table-checkable" id="key_attributes">
                     <thead>
                        <tr>
                           <th>Sr. No.</th>
                           <th>Title</th>
                     ...
                     <tbody>
                        @php
                          $i = 1;
                        @endphp
                        @foreach($keyAttribute as $kAtt)
                        <tr>
                          <td>{{$i++}}</td>
                           <td>{{ $kAtt->title }}</td>
                           <td>
                              <img class="image_fade" src="{{ asset('storage/keyAttributeImg/'.$kAtt->image)}}" style="width:100px; height: 68px;">
                           </td>
                           <td><span class="label label-lg font-weight-bold {{ $kAtt->is_deleted == 0 ? 'label-light-primary' : 'label-light-danger' }} label-inline">{{ $kAtt->is_deleted == 0 ? 'No' : 'Yes' }}</span></td>
                           <td>
                              <a href="javascript:;" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details" onclick="editKeyAttribute({{$kAtt->id}})">
...
<script type="text/javascript">
   ...
   function editKeyAttribute($id){
      $('#image_view').empty();

     $.get("/admin/key_attributes/edit/"+$id, function( data, status ) {

         var imgSrc = "{{ asset('storage/keyAttributeImg')}}"+"/"+data.image;
         console.log(imgSrc);
         ...
         if (data.is_deleted == 0) {
          ...
          $('#keyId').val(data.id);
          $('#form-heading').text('Edit Hotel Key Attribute');
          $('.btn-primary').text('Update');
          $('#form1').attr('action', '{{ url("/admin/key_attributes/update") }}');
     });
   }
</script>
@endsection