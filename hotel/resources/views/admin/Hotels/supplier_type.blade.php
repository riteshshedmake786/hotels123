            ...
            <div class="row">
                <div class="col-lg-5">
                    ...
                    <div class="card card-custom gutter-b example example-compact">
                        ...
                        <!--begin::Form-->
                        <form class="form" method="post" action="{{ url('admin/supplier_type/add') }}" id="form1"
                    ...
                    <div class="card card-custom gutter-b">
                        <div class="card-header flex-wrap py-3">
                            <div class="card-title">
                                <h3 class="card-label">Supplier Type List</h3>
                            </div>
                        </div>
                        <div class="card-body">
                            <!--begin: Datatable-->
                            <table class="table table-bordered table-checkable" id="event_attributes">
                                ...
                                @php
                                    $i = 1;
                                @endphp
                                @foreach($supplierType as $sAtt)
                                    <tr>
                                        <td>{{$i++}}</td>
                                        <td>{{ $sAtt->title }}</td>
                                        <td>
                                            <img class="image_fade"
                                                 src="{{ asset('storage/supplierTypeImg/'.$sAtt->image)}}"
                                                 style="width:100px; height: 68px;">
                                        </td>
                                        <td><span
                                                class="label label-lg font-weight-bold {{ $sAtt->is_deleted == 0 ? 'label-light-primary' : 'label-light-danger' }} label-inline">{{ $sAtt->is_deleted == 0 ? 'No' : 'Yes' }}</span>
                                        </td>
                                        <td>
                                            <a href="javascript:;" class="btn btn-sm btn-clean btn-icon mr-2"
                                               title="Edit details" onclick="editEventType({{$sAtt->id}})">
...
@section('pageScript')
    <script type="text/javascript">
        ...
        function editEventType($id) {
            ...
            //$.get("/admin/supplier_type/edit/" + $id, function (data, status) {
			$.get("{{ url('/admin/supplier_type/edit/')}}/" + $id, function (data, status) {	

                var imgSrc = "{{ asset('storage/supplierTypeImg')}}" + "/" + data.image;
                console.log(imgSrc);
                ...
                $('#form1').attr('action', '{{ url("/admin/supplier_type/update") }}');
            });
        }
    </script>
@endsection
