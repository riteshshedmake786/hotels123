    ...
    <div class="card card-custom gutter-b example example-compact">
     <div class="card-header">
      <h3 id="form-heading" class="card-title">
       Add Hotel Capacity Attribute
     </h3>
   </div>
   <!--begin::Form-->
   <form class="form" method="post" action="{{ url('admin/capacity_attributes/add') }}" id="form1" enctype="multipart/form-data">
    {{ csrf_field() }}
       ...
       <label class="checkbox checkbox-outline checkbox-outline-2x checkbox-primary checkbox-lg">
        ...
        <span></span>
                  ...
                  <div class="card card-custom gutter-b" >
                   <div class="card-header flex-wrap py-3">
                    <div class="card-title">
                     <h3 class="card-label">Capacity Attributes List</h3>
                   </div>
                 </div>
                 <div class="card-body">
                  <!--begin: Datatable-->
                 ...
                 <tbody>
                  ...
                  $i = 1;
                  @endphp
                  @foreach($capacityAttribute as $cAtt)
                  <tr>
                   <td>{{$i++}}</td>
                   <td>{{ $cAtt->title }}</td>
                   <td>
                    <img class="image_fade" src="{{ asset('storage/capacityAttributeImg/'.$cAtt->image)}}" style="width:100px; height: 68px;">
                  </td>
                  ...
                  <td>
                    <a href="javascript:;" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details" onclick="editCapacityAttribute({{$cAtt->id}})">
           ...
           <script type="text/javascript">
             ...
             function editCapacityAttribute($id){
              $('#image_view').empty();

              $.get("/admin/capacity_attributes/edit/"+$id, function( data, status ) {

               var imgSrc = "{{ asset('storage/capacityAttributeImg')}}"+"/"+data.image;
               console.log(imgSrc);
          ...
          $('#capId').val(data.id);
          $('#form-heading').text('Edit Hotel Capacity Attribute');
          $('.btn-primary').text('Update');
          $('#form1').attr('action', '{{ url("/admin/capacity_attributes/update") }}');
        });
            }
          </script>
          @endsection