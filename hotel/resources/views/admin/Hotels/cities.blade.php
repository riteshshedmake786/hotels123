    ...
    <div class="card card-custom gutter-b example example-compact">
     <div class="card-header">
      <h3 id="form-heading" class="card-title">
       Add Hotel City Attribute
     </h3>
   </div>
   <!--begin::Form-->
   <form class="form" method="post" action="{{ url('admin/cities/add') }}" id="form1" enctype="multipart/form-data">
    {{ csrf_field() }}
    <input type="hidden" id="citId" name="id">
    <div class="card-body">
     <div class="form-group">
      <label>Name</label>
      <input type="text" class="form-control" name="name" id="name" placeholder="City Name" value="{{ old('name') }}" required>
      @if ($errors->has('name'))
...
<div class="col-lg-7">
  ...
  <div class="card card-custom gutter-b" >
   <div class="card-header flex-wrap py-3">
    <div class="card-title">
     <h3 class="card-label">Cities Attributes List</h3>
   </div>
 </div>
 <div class="card-body">
  <!--begin: Datatable-->
  <table class="table table-bordered table-checkable" id="capacity_attributes">
   <thead>
    <tr>
     <th>Sr. No.</th>
     <th>Name</th>
 ...
 <tbody>
  ...
  <tr>
    ...
    <td>{{ $cAtt->state }}</td>
    <td><span class="label label-lg font-weight-bold {{ $cAtt->status === 'APPROVED' ? 'label-light-primary' : 'label-light-danger' }}  label-inline">{{ $cAtt->status }}</span></td>
    ...
    <td>
     <a href="javascript:;" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details" onclick="editCity({{$cAtt->id}})">
...
<script type="text/javascript">
  ...
  function editCity($id){
     $.get("/admin/cities/edit/"+$id, function( data, status ) {
         ...
         if (data.is_deleted == 0) {
          ...
          $('#citId').val(data.id);
          $('#form-heading').text('Edit Hotel City Attribute');
          $('.btn-primary').text('Update');
          $('#form1').attr('action', '{{ url("/admin/cities/update") }}');
     });
   }
</script>
@endsection