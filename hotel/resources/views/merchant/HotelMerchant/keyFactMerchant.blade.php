    ...
    <div class="d-flex align-items-center flex-wrap mr-1">
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex align-items-center font-weight-bold my-2">
          ...
          Hotel Details                           </a>
          <!--end::Item-->
          <!--begin::Item--> <span class="label label-dot label-sm bg-dark opacity-75 mx-3"></span> <a href="{{ url('merchant/hotelList') }}" class="text-dark text-hover-dark opacity-75 hover-opacity-100">Hotel List</a>
          <span class="label label-dot label-sm bg-dark opacity-75 mx-3"></span> <a href="3" class="text-dark text-hover-dark opacity-75 hover-opacity-100">
          Hotel View</a>
          <!--end::Item-->
        </div>
        <!--end::Breadcrumb-->
    ...
    <div class="d-flex align-items-center flex-wrap mr-1">
      ...
      <div class="d-flex flex-column">
        ...
        <div class="d-flex align-items-center font-weight-bold my-2">
          ...
          <!--end::Item-->
          <!--begin::Item-->
          <span class="label label-dot label-sm bg-dark opacity-75 mx-3"></span> <a href="{{ url('merchant/hotelMerchantList') }}" class="text-dark text-hover-dark opacity-75 hover-opacity-100">
          Hotel List</a>
  ...
  <div class="card-body">
    ...
    <div class="example">
      <ul class="nav nav-tabs" id="myTab" role="tablist">
        <!-- <li class="nav-item">
          <a class="nav-link" id="HotelIncludes"  href="{{url('merchant/hotel/hotelMerchantView/'.$hotelsData->id)}}"> <span class="nav-icon"><i class="fas fa-house-user"></i></span> <span class="nav-text">Hotel Venue List</span> </a>
        </li> -->
        <li class="nav-item ">
        ...
        <div class="card card-custom gutter-b" style="box-shadow: none;">
          <div class="card-header flex-wrap py-3">
            <div class="card-title">
              ...
              <a href="#" class="btn btn-primary font-weight-bolder" style=" height: 37px;" data-toggle="modal" data-target="#addKeyFactMerchantModal">
                <span class="fas fa-plus-circle"></span>&nbsp;Add Key Fact Merchant
              </a>
            </div>
            <div class="card-body">
              <!--begin: Datatable-->
              <table class="table table-bordered table-checkable" id="hotelKeyList">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-center">{{$list->key_name}}</td>
                    <td class="text-center"><img src="{{ asset('storage/keyAttributeImg/'.$list->ket_image)}}" alt="image" style="width: 150px; height: 100px;" /></td>
                    <td>{{$list->description}}</td>
                    <td>
                      <a href="javascript:;" class="btn btn-sm btn-clean btn-icon mr-2" title="View details" onclick="editKeyFact({{$list->hotel_id}}, {{$list->id}})">
...
<!-- Add Modal -->
<div class="modal fade" id="addKeyFactMerchantModal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Key Fact Merchant Form</h5>
     ...
     <div class="modal-body">
       ...
       <form action="{{url('merchant/hotel/submitkeyFactMerchant')}}" method="post">
        {{ csrf_field() }}
...
<div id="hotelImage" class="modal" tabindex="" style=" padding-right: 17px;">
  ...
  <form class="form" action="{{ url('merchant/hotel/editHotelsGalleryDataMerchant') }}" method="post" enctype="multipart/form-data">
      {{ csrf_field() }}
...
<div class="modal fade" id="kfEditModal" tabindex="-1" role="dialog" data-backdrop="static"
    aria-hidden="true" aria-labelledby="staticBackdrop">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit Key Fact Merchant Form</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                   <i aria-hidden="true" class="ki ki-close"></i>
                 </button>
            </div>
  ...
  $(document).ready(function(){
    $('#addSocial').on('click',function(){
      ...
      if(counter < 5){
        ...
        $('#social_input').append('<div class="row div_f_'+f+'" data-id="'+f+'"><div class="form-group validated col-md-5"><label class="form-control-label" for="key_attribute_id">Title</label><select id="key_attribute_id" class="form-control" name="key_attribute_id[]" required=""><option value="">Select</option>@foreach($key as $ci)<option value="{{$ci->id}}">{{$ci->title}}</option>@endforeach</select></div><div class="form-group col-md-6" id="website_input"><label for="exampleTextarea">Description</label><textarea class="form-control" id="exampleTextarea" name="description[]" rows="2"></textarea></div><div class="form-group m-auto"><button type="button" class="btn btn-outline-danger btn-icon btn-circle" onclick="dltTrTrabgle('+f+')"><i class="flaticon2-cancel-music"></i></button> </div></div>');
        f++;
      }  
    });
...
if(counter >= 0){
  ...
  function editKeyFact($hotel_id, $id){
    var hotel_id = $hotel_id;
    var id = $id;

    console.log(hotel_id);
    console.log(id);
    $.post("edit_kf_merchant", {hotel_id: hotel_id, id: id , _token: '{!! csrf_token() !!}'})
    .done(function( data ) {
        if(data.html != '') {
          console.log('hi json = ' + data.html)
          $('#kfEditModal .modal-body').html('');
          $('#kfEditModal .modal-body').html(data.html);
          $('#kfEditModal').modal(
            {
              backdrop: 'static',
              keyboard: false,
...
$(document).on('click', '#saveEdit', function(){
  ...
  var id =$('#kfEditModal [name="id"]').val();
  var hotel_id =$('#kfEditModal [name="hotel_id"]').val();
  var key_attribute_id= $('#kfEditModal [name="key_attribute_id"]').val();
  var description= $('#kfEditModal [name="description"]').val();
  console.log(id, hotel_id, key_attribute_id, description);

  if (key_attribute_id == "") {
    swal.fire('Please select the title', 'Required error', 'error');
  } else if (description == ""){
    swal.fire('Please enter the description', 'Required error', 'error');
  } else {
        $.post( "save_edit_kf_modal_merchant", { id: id, hotel_id: hotel_id, key_attribute_id:key_attribute_id, description: description, _token: '{!! csrf_token() !!}' })
        .done(function( data ) {
          $('#kfEditModal textarea').val('');
            $('#kfEditModal select').selectpicker('deselectAll').selectpicker('refresh');
            $('#kfEditModal').modal('hide');
            $("#editData").remove();
            location.reload();
          })
        .fail(function() {
